    ...
    <h5>FlashCards
        <!-- <button type="button" class="float-right btn btn-icon bg-gradient-secondary reset_form" data-bs-toggle="modal" data-bs-target="#addForm">
          Add
            ...
            <a class="float-right btn btn-icon bg-gradient-secondary" href="{{ url('flashcard/create/') }}"> Add </a>
              ...
              <tbody>  
                @foreach($flashcard as $key=> $val)   
                    <tr>
                        ...
                        <td >
                            <a class="btn bg-gradient-secondary btn-rounded btn-condensed btn-sm s_btn1 " href="{{url('flashcard')}}/{{$val->id}}/edit" ><i class="fa fa-pencil"></i></a>
                            <form action="{{url('flashcard')}}/{{$val->id}}" method="post" style="display: inline-block;">
                                @csrf
                                @method('delete')
